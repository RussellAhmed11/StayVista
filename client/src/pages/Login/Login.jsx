...
const Login = () => {
  ...
  const handleRetsetPassword = async () => {
    if(!email) return toast.error('please write your email')
    try {
      await resetPassword(email);
      toast.success("Request success! cheack your email now");
      setLoading(false)
    } catch (err) {
      setLoading(false)
     console.log(err);
    ...
    <div className='flex justify-center items-center min-h-screen'>
      <div className='flex flex-col max-w-md p-6 rounded-md sm:p-10 bg-gray-100 text-gray-900'>
        ...
        <div className='space-y-1'>
          <button onClick={handleRetsetPassword} className='text-xs hover:underline hover:text-rose-500 text-gray-400'>
            Forgot password?
          </button>
        </div>
